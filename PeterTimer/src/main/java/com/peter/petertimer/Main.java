...
//			System.out.println(WARNSEQ + "You are running a dev version of this plugin");
		...
		if(!Bukkit.getVersion().contains(pdf.getAPIVersion())) {
			logger.log(Level.WARNING, "You are running a unsuported server version, check for a new version for you server. (Server:{0}, Plugin:{1})", new Object[]{Bukkit.getVersion(), pdf.getAPIVersion()});
		}
